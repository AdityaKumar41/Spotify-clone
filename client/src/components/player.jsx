...
const Player = () => {
  ...
  useEffect(() => {
    if (seekBar.current) {
      seekBar.current.style.width = `${(time.currentTime.seconds + time.currentTime.minute * 60) / (time.totalTime.seconds + time.totalTime.minute * 60) * 100}%`;
    }
    if (fullScreenSeekBar.current) {
      fullScreenSeekBar.current.style.width = `${(time.currentTime.seconds + time.currentTime.minute * 60) / (time.totalTime.seconds + time.totalTime.minute * 60) * 100}%`;
    }
  }, [time]);
      ...
      {isFullScreen && (
        <div className="fixed inset-0 z-50 text-white">
          ...
          <div 
            ...
            style={{
              background: 'linear-gradient(rgba(0, 0, 0, 0) 0%, rgb(0, 0, 0) 15%, rgb(0, 0, 0) 100%)'
            }}
          />
          ...
          <div className="relative h-full flex flex-col p-8 z-10">
            ...
            <div className="flex flex-col items-center gap-4 mb-8 backdrop-blur-sm bg-black/20 py-6 rounded-lg">
              <div className="flex items-center gap-8">
                ...
                <img className="w-6 cursor-pointer" src={assets.prev_icon} alt="previous" onClick={previous} />
                {playStatus ? (
                ...
                <img className="w-6 cursor-pointer" src={assets.next_icon} alt="next" onClick={next} />
                <img className="w-6 cursor-pointer" src={assets.loop_icon} alt="loop" />
              </div>
              
              {/* Seek Bar */}
              <div className="w-full max-w-2xl flex items-center gap-4">
                <p>{String(time.currentTime.minute).padStart(2, "0")}:{String(time.currentTime.seconds).padStart(2, "0")}</p>
                <div ref={fullScreenSeekBg} onClick={seekSongFullScreen} className="flex-1 bg-gray-600 rounded-full cursor-pointer">
                  <hr ref={fullScreenSeekBar} className="h-1 border-none w-0 bg-white rounded-full cursor-pointer" />
                </div>
                <p>{String(time.totalTime.minute).padStart(2, "0")}:{String(time.totalTime.seconds).padStart(2, "0")}</p>
              </div>
            </div>
          </div>
        </div>
